...
| **数组Arrays创建** |                |
| static  Stream stream(T[] array): | 顺序流 |
| public static IntStream stream(int[] array) | 顺序流 |
| public static LongStream stream(long[] array) | 顺序流 |
| public static DoubleStream stream(double[] array) | 顺序流 |
| **值创建** | |
| public static Stream of(T... values) | 顺序流 |
| 函数创建无限流 | |
| public static Stream iterate(final T seed, final UnaryOperator f) | 顺序流 |
| public static Stream generate(Supplier s) | 顺序流 |
...
| **归约**                         | 备注：map 和 reduce 的连接通常称为 map-reduce 模式，因 Google 用它 来进行网络搜索而出名。 |
...
| toSet             | Set<T>               | 把流中元素收集到Set                                          | Set<Employee>  emps= list.stream().collect(Collectors.toSet()); toCollection Collection |
